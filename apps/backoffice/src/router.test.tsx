...
test("re-renders with the new pathname after navigate() pushes a history entry", async () => {
  ...
  navigate("/help/catalogo");

  await expect.element(screen.getByTestId("route")).toHaveTextContent("/help/catalogo");
...
test("a replacing navigate() swaps the current history entry instead of adding one", async () => {
  ...
  navigate("/help/ventas", { replace: true });

  await expect.element(screen.getByTestId("route")).toHaveTextContent("/help/ventas");
  expect(window.history.length).toBe(lengthBefore);
