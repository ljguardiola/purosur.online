...
export interface BuildAppOptions<TQueryResult extends PgQueryResultHKT = PostgresJsQueryResultHKT> {
  ...
  /**
   * Registers `GET /users` and `GET /users/:id`, the backoffice Users screen's read side (issue
   * #247): both are Administrator-only and scoped to the session's own branch, the same
