...
// token-in-query would otherwise travel), never a token, key, secret, password or a credential
// embedded in a connection string. What reaches Sentry is the error type, its stack trace, and
// opaque identifiers.
...
function redactString(value: string): string {
  return value.replace(URL_PATTERN, redactUrl).replace(BEARER_TOKEN_PATTERN, REDACTED);
}
