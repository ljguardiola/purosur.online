...
- Actions variable `LINUX_RUNNER`: where the Linux jobs run, as a JSON `runs-on` value; a value that is not valid JSON stops every Linux job, required checks included. `["self-hosted","purosur-linux"]` sends them to the project's own runners; deleting the variable sends them back to GitHub-hosted `ubuntu-24.04`, for example while those runners are down. The staging deploy always runs on GitHub-hosted runners. The own runners execute pull request code on a persistent machine, which is acceptable only while the repository is private; they need git, Docker and Playwright's system libraries installed, the libraries matching the pinned Playwright version.
- Actions variable `WINDOWS_RUNNER`: where a pull request's register packaging runs, as a JSON `runs-on` value, with the same format rule. `["self-hosted","purosur-windows"]` sends it to the project's own Windows runner; deleting the variable sends it back to GitHub-hosted `windows-latest`. Packaging on a push to main, whose staging installer is distributed, and the independent build that checks it always run on GitHub-hosted runners. Like the Linux ones, this runner executes pull request code on a persistent machine, which is acceptable only while the repository is private. It needs git, and it must run in a logged-on, never-locked desktop session rather than as a service: the register's end-to-end tests open real windows, which a service's session never shows. Each end-to-end run leaves a temporary folder and a `purosur-pos-e2e*` app-data folder behind, so the runner host has to clear them, for example at logon.
