...
- Actions variable `LINUX_RUNNER`: where the Linux jobs run, as a JSON `runs-on` value; a value that is not valid JSON stops every Linux job, required checks included. `["self-hosted","purosur-linux"]` sends them to the project's own runners; deleting the variable sends them back to GitHub-hosted `ubuntu-24.04`, for example while those runners are down. Pull requests opened by Dependabot always run on GitHub-hosted runners, whoever reopens or updates them: installing a dependency update runs third-party install scripts, which reach the own runners only once a person has reviewed and merged the update. The own runners execute pull request code on a persistent machine, which is acceptable only while the repository is private; they need git, Docker and Playwright's system libraries installed, the libraries matching the pinned Playwright version.
- Actions variable `WINDOWS_RUNNER`: where the Windows jobs (register packaging and its independent build) run, as a JSON `runs-on` value, with the same format rule and the same Dependabot exception. `["self-hosted","purosur-windows"]` sends them to the project's own Windows runner; deleting the variable sends them back to GitHub-hosted `windows-latest`. On the one own Windows runner the independent build shares the packaging job's host and caches, so its agreement then shows the build is deterministic, not that the host is clean. Like the Linux ones, this runner executes pull request code on a persistent machine, which is acceptable only while the repository is private. It needs git, and it must run in a logged-on, never-locked desktop session rather than as a service: the register's end-to-end tests open real windows, which a service's session never shows. Each end-to-end run leaves a temporary folder and a `purosur-pos-e2e*` app-data folder behind, so the runner host has to clear them, for example at logon.
